...
async def test_get_memory_returns_existing(service, test_namespace):
    ...
    service.update_memory(test_namespace, "preferences", content)
    result = service.get_memory(test_namespace, "preferences")
...
async def test_search_memories_returns_all(service, test_namespace):
    ...
    service.update_memory(test_namespace, "key1", "content1")
    service.update_memory(test_namespace, "key2", "content2")

    results = service.search_memories(test_namespace, limit=10)
...
async def test_search_memories_respects_limit(service, test_namespace):
    ...
    for i in range(5):
        service.update_memory(test_namespace, f"key{i}", f"c{i}")

    results = service.search_memories(test_namespace, limit=3)
...
async def test_get_memory_without_default_returns_none(
    ...
    result = service.get_memory(test_namespace, "missing")
...
async def test_memory_isolation_across_namespaces(service):
    ...
    service.update_memory(ns1, "pref", "user1_data")
    service.update_memory(ns2, "pref", "user2_data")

    result1 = service.get_memory(ns1, "pref")
    result2 = service.get_memory(ns2, "pref")
