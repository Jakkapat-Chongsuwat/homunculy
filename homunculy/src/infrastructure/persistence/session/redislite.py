"""Embedded Redis session store (redislite)."""
...
class RedisliteSessionStore(SessionStorePort):
    """Embedded Redis session store for routing."""

    def __init__(self, db_file: str) -> None:
        if Redis is None:
            raise RuntimeError("redislite is not available on this platform")
        self._client = Redis(db_file)
