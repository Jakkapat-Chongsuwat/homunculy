...
from .exceptions import TTSServiceError, TTSSynthesisError
...
class ElevenLabsTTSService(TTSService):
    ...
    async def synthesize(
        ...
        similarity_boost = (
            ...
            else tts_settings.default_similarity_boost
        )
        style = style if style is not None else tts_settings.default_style
        use_speaker_boost = (
            use_speaker_boost
            if use_speaker_boost is not None
            else tts_settings.default_use_speaker_boost
        )

        try:
            logger.info(
                "Synthesizing speech", voice_id=voice_id, model=model_id, text_length=len(text)
            )
            ...
            logger.info(
                "Speech synthesis completed", audio_size=len(audio_bytes), voice_id=voice_id
            )

            return audio_bytes

        except Exception as e:
            logger.error("Speech synthesis failed", error=str(e), voice_id=voice_id, exc_info=True)
    ...
    def stream(
        ...
        logger.info(
            "Starting TTS streaming", voice_id=voice_id, model=model_id, text_length=len(text)
        )
