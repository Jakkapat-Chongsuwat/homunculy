...
This repository uses SQLAlchemy ORM for agent persistence operations.
...
class SQLAlchemyAgentRepository(AgentRepository):
    ...
    def _to_domain(self, model: AgentModel) -> Agent:
        ...
        # Type ignores needed because SQLAlchemy returns Column types at class level
        # but actual values at instance level
        config_dict: dict = model.configuration  # type: ignore
        ...
        return Agent(
            id=str(model.id),  # type: ignore
            name=str(model.name),  # type: ignore
            configuration=configuration,
            status=AgentStatus(model.status),  # type: ignore
            is_active=bool(model.is_active),  # type: ignore
            created_at=model.created_at,  # type: ignore
            updated_at=model.updated_at,  # type: ignore
        )
    
    def _to_model(self, agent: Agent) -> dict:
        """Convert domain entity to SQLAlchemy model dict."""
        return {
            "id": agent.id,
            "name": agent.name,
