...
class AgentPersonalityRequest(BaseModel):
    """Agent personality request."""

    name: str = Field(description="Agent name/identity")
    description: str = Field(description="Brief description of agent's purpose")
    traits: dict = Field(
        default_factory=dict,
        description="Personality traits (e.g., {'humor': 'witty', 'tone': 'professional'})",
    )
    mood: str = Field(
        default="neutral", description="Current mood state (e.g., neutral, excited, serious)"
    )
...
class AgentConfigurationRequest(BaseModel):
    ...
    provider: str = Field(
        default="langraph", description="AI orchestration provider (currently only 'langraph')"
    )
    model_name: str = Field(
        default="gpt-4", description="LLM model to use (e.g., gpt-4, gpt-3.5-turbo)"
    )
...
class ExecuteChatRequest(BaseModel):
    """Stateless agent execution request."""
    ...
    configuration: AgentConfigurationRequest = Field(
        description="Complete agent configuration (not stored)"
    )
    context: dict = Field(
        default_factory=dict,
        description="Additional conversation context (session_id, metadata, etc.)",
    )
    include_audio: bool = Field(
        default=True,
        description="Generate and include TTS audio in response (default: enabled if TTS available)",
    )
